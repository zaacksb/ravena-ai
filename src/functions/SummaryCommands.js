...
const commands = [
  {
    ...
    method: async (bot, message, args, group) => {
      await summarizeConversation(bot, message, args, group);
    }
  },
  {
    name: 'interagir',
    ...
    method: async (bot, message, args, group) => {
      await interactWithConversation(bot, message, args, group);
    }
  }
];
...
async function summarizeConversation(bot, message, args, group) {
  try {
    ...
    if (!recentMessages || recentMessages.length === 0) {
      await bot.sendMessage(message.group, 'Nenhuma mensagem recente para resumir.');
      return;
    }
    
    // Envia indicador de digitação
    try {
      await bot.client.sendPresenceUpdate('composing', message.group);
    } catch (error) {
...
Resumo:`;
    ...
    if (!summary) {
      await bot.sendMessage(message.group, 'Falha ao gerar resumo. Por favor, tente novamente.');
      return;
    }
    
    // Envia o resumo
    await bot.sendMessage(message.group, `📋 *Resumo da conversa:*\n\n${summary}`);
    
    logger.info(`Resumo de conversa enviado com sucesso para ${message.group}`);
  } catch (error) {
    logger.error('Erro ao resumir conversa:', error);
    await bot.sendMessage(message.group, 'Erro ao gerar resumo. Por favor, tente novamente.');
  }
}
...
async function interactWithConversation(bot, message, args, group) {
  try {
    ...
    if (!recentMessages || recentMessages.length === 0) {
      await bot.sendMessage(message.group, 'Nenhuma mensagem recente para interagir.');
      return;
    }
...
Uma mensagem curta para interagir:`;
    ...
    if (!interaction) {
      await bot.sendMessage(message.group, 'Falha ao gerar mensagem. Por favor, tente novamente.');
      return;
    }
    
    // Envia a mensagem de interação
    await bot.sendMessage(message.group, interaction);
    
    logger.info(`Mensagem de interação enviada com sucesso para ${message.group}`);
  } catch (error) {
    logger.error('Erro ao gerar interação:', error);
    await bot.sendMessage(message.group, 'Erro ao gerar mensagem. Por favor, tente novamente.');
  }
}
