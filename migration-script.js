 ...
 * 
 * Modificação: Quando há mais de um comando com o mesmo nome em um grupo,
 * apenas o mais recente (com ID maior) é considerado.
 */
...
function convertCommand(oldCommand) {
  // Remove o prefixo "!" do início do comando se existir
  const commandText = oldCommand.msgInicio.startsWith('!') 
    ? oldCommand.msgInicio.substring(1) 
    : oldCommand.msgInicio;
    
  const newCommand = {
    startsWith: commandText,
    responses: oldCommand.respostas.map(response => {
      // Formato 1: "legenda img-nomeArquivo.jpg"
      const formatoLegendaPrimeiro = response.match(/(.*?)\s+(img|gif|vid|sfx|sticker|stickerguif|stickerimag)-([^\\/:*?"<>|\r\n]+)$/);
      
      // Formato 2: "img-nomeArquivo.jpg legenda"
      const formatoArquivoPrimeiro = response.match(/^(img|gif|vid|sfx|sticker|stickerguif|stickerimag)-([^\\/:*?"<>|\r\n]+)(\s+(.*))?/);
      
      if (formatoLegendaPrimeiro) {
        const caption = formatoLegendaPrimeiro[1].trim();
        const mediaType = formatoLegendaPrimeiro[2];
        const mediaFileName = formatoLegendaPrimeiro[3];
        const newMediaType = MEDIA_TYPE_MAP[mediaType] || 'image';
        
        return `{${newMediaType}-${mediaFileName}}${caption ? ' ' + caption : ''}`;
      } else if (formatoArquivoPrimeiro) {
        const mediaType = formatoArquivoPrimeiro[1];
        const mediaFileName = formatoArquivoPrimeiro[2];
        const caption = formatoArquivoPrimeiro[4] ? formatoArquivoPrimeiro[4].trim() : '';
        const newMediaType = MEDIA_TYPE_MAP[mediaType] || 'image';
    ...
    metadata: {
      ...
      createdAt: Date.now(),
      // Adiciona ID do comando original para ordenação posterior
      originalId: oldCommand.id || Date.now()
    },
...
async function processCustomCommands() {
  // Mapa para armazenar comandos por grupo, com comandos organizados por nome
  // Estrutura: Map<groupId, Map<commandName, Array<command>>>
  const commandsByGroupAndName = new Map();
  ...
  for (const oldBasePath of OLD_BASE_PATHS) {
    ...
    for (const oldCommand of oldCommandsData.frases) {
      ...
      for (const channel of oldCommand.canais) {
        ...
        // Inicializa mapa de comandos por nome para o grupo se necessário
        if (!commandsByGroupAndName.has(groupId)) {
          commandsByGroupAndName.set(groupId, new Map());
        }
        
        const groupCommands = commandsByGroupAndName.get(groupId);
        const commandName = newCommand.startsWith;
        
        // Inicializa array de comandos com esse nome se necessário
        if (!groupCommands.has(commandName)) {
          groupCommands.set(commandName, []);
        }
        
        // Adiciona o comando ao array de comandos com esse nome
        groupCommands.get(commandName).push(newCommand);
        
        // Verifica e processa arquivos de mídia
        for (const response of oldCommand.respostas) {
          // Verifica os dois formatos possíveis
          let mediaType, mediaFileName;
          ...
          if (formatoLegendaPrimeiro) {
            mediaType = formatoLegendaPrimeiro[2];
            mediaFileName = formatoLegendaPrimeiro[3];
          } else if (formatoArquivoPrimeiro) {
            mediaType = formatoArquivoPrimeiro[1];
            mediaFileName = formatoArquivoPrimeiro[2];
          } else {
            continue; // Não é um comando de mídia
          }
  ...
  // Processa o mapa de comandos para selecionar apenas os mais recentes com o mesmo nome
  const finalCommandsByGroup = new Map();
  
  for (const [groupId, commandsByName] of commandsByGroupAndName.entries()) {
    const finalCommands = [];
    
    // Para cada nome de comando neste grupo
    for (const [commandName, commands] of commandsByName.entries()) {
      if (commands.length > 1) {
        // Se houver múltiplos comandos com o mesmo nome, ordena pelo ID original (maior = mais recente)
        commands.sort((a, b) => (b.metadata.originalId || 0) - (a.metadata.originalId || 0));
        
        // Adiciona apenas o comando mais recente
        const mostRecentCommand = commands[0];
        console.log(`Grupo ${groupId}: Mantendo apenas o comando mais recente para "${commandName}" (ID: ${mostRecentCommand.metadata.originalId})`);
        
        // Remove o ID original antes de salvar
        delete mostRecentCommand.metadata.originalId;
        finalCommands.push(mostRecentCommand);
      } else if (commands.length === 1) {
        // Se houver apenas um comando com este nome, adiciona-o
        const command = commands[0];
        // Remove o ID original antes de salvar
        delete command.metadata.originalId;
        finalCommands.push(command);
      }
    }
    
    finalCommandsByGroup.set(groupId, finalCommands);
  }
  
  // Salva os arquivos de comandos personalizados para cada grupo
  for (const [groupId, commands] of finalCommandsByGroup.entries()) {
